...
void GameEngine::Transform::SetPosition(const float x, const float y, const float z)
{
	...
	m_Position.y = y;
	m_Position.z = z;
	m_TranslateMatrix = MathHelper::Matrix::CreateTranslation(m_Position);
}

void GameEngine::Transform::Translate(float x, float y)
{
	m_TranslateMatrix = MathHelper::Matrix::CreateTranslation(x, y);
}

void GameEngine::Transform::SetRotation(float angle)
{
	m_RotateMatrix = MathHelper::Matrix::CreateRotation(angle);
}

void GameEngine::Transform::SetScale(float xScale, float yScale)
{
	m_ScaleMatrix = MathHelper::Matrix::CreateScale(xScale, yScale);
}

void GameEngine::Transform::Update()
{
	MathHelper::Matrix worldMatrix = m_TranslateMatrix * m_RotateMatrix * m_ScaleMatrix;
...
Transform GameEngine::Transform::operator+(const Transform& other)
{
	...
	result.m_TranslateMatrix = MathHelper::Matrix::CreateTranslation(result.m_Position);
